...
Times_csv = read.csv("data/germany-deaths-16april.csv")[2]
Times_mat = floor(Times_csv[Times_csv>=1])
Times_mat = t(sapply(1:max(Times_csv), function(t) {cbind(t, length(which(Times_mat == t)))}))
Times = list(Times_mat)
...
Times_utiles=lapply(1:M,function(m){u_m=Times[[m]]; u_m=u_m[u_m[,1] > Tinf-max(smax) ,];  u_m=u_m[u_m[,1] <= Tmax ,]})
...
hyperParam_prior=list(M=M,mu_lalpha=mu_lalpha,s_lalpha=s_lalpha,mu_lnu=mu_lnu,s_lnu=s_lnu, 
                      mu_lbeta=mu_lbeta,s_lbeta=s_lbeta,mu_llag=mu_llag,s_llag=s_llag,p_Z=p_Z,a_s=2,pi_0=pi_0)
...
h_init$s =   list(); 
for (p in  1:(M^2)) {h_init$s[[p]] =   seq(0,h_init$smax[p],length =   h_init$K[p] + 1)}
...
nu_init  =    vapply(1:M,function(m){length(Times_obs[[m]][,1])/(data$Tmax[m] - data$Tinf[m])},1)
beta_init = rep(1,M)
lag_init = rep(3,M)

theta_init =   list(h =   h_init,nu =   nu_init, beta = beta_init, lag = lag_init)
INPUT =   list(theta =   theta_init)


#------- Running MCMC (Reversible Jump) 
resMCMC = RJ_Kernel_regular_s_inter_dt(data,INPUT,hyperParam_prior,par_algo_MCMC)
# resMCMC$INPUT <- INPUT
