...
RJ_Kernel_regular_s_inter_dt = function(data, INPUT, hyperParam_prior, par_algo_MCMC,op.plot = FALSE, h_vrai = c(), op.test.on.rho = FALSE) { 
  #op_in_SMC=FALSE
  ...
  # Intervention time plus lag
  lag_t1 = t1 + lag
  
  # Before intervention + lag
  data$Times_obs_1=lapply(1:M,function(m){u_m=Times[[m]]; u_m = u_m[u_m[,1]>Tinf ,]; u_m=u_m[u_m[,1] <= lag_t1 ,]})
  data$Times_utiles_1=lapply(1:M,function(m){u_m=Times[[m]]; u_m=u_m[u_m[,1] > Tinf-max(smax) ,];  u_m=u_m[u_m[,1] <= lag_t1 ,]})
  data$obs_utiles_1 = calc_absc_utiles_dt(Times_utiles_1,Times_obs_1,smax)
  
  # After intervention + lag
  data$Times_obs_2=lapply(1:M,function(m){u_m=Times[[m]]; u_m = u_m[u_m[,1]>min(Tmax,lag_t1) ,]; u_m=u_m[u_m[,1] <= Tmax ,]})
  data$Times_utiles_2=lapply(1:M,function(m){u_m=Times[[m]]; u_m=u_m[u_m[,1] > min(Tmax,lag_t1)-max(smax) ,];  u_m=u_m[u_m[,1] <= Tmax ,]})
  data$obs_utiles_2 = calc_absc_utiles_dt(Times_utiles_2,Times_obs_2,smax)
  
  # Calculate log likelihood for intervention
  log_lik = log_likelihood_multidim_inter_dt(data, h, nu, beta, op ='vec')
