...
RJ_Kernel_regular_s_positive_intensity = function(data, INPUT, hyperParam_prior, par_algo_MCMC,op.plot = FALSE, h_vrai = c(), op.test.on.rho = FALSE) { 
#op_in_SMC=FALSE
...
seq_MCMC$lambda_K = rep(0,N_MCMC); seq_MCMC$lambda_K[1] <- lambda_K; 
...
########################################################
  ...
  for (p in op_echan$delta) { 
    ...
    if (h$delta[p] == 1) { 
      ...
      h_c$s[[p]] <- seq(0,smax[p],len = h_c$K[p] + 1)
    }
      
    if (op.test.on.rho == TRUE) {test_rho_h_c <- calcul_spectral_norm_rho(h_c) < 0.999} else {test_rho_h_c = TRUE}
    if (!test_rho_h_c) {test_accept = FALSE}
    if (test_rho_h_c) { 
      ...
      if (move == 'increase') { 
        h_small = h; h_big = h_c; 
        q_small_to_big = dpois(h_big$K[p] - 1,lambda_K,log = TRUE) + dprior_lalpha(h_big$lalpha[[p]],mu_lalpha,s_lalpha,p_Z,log = TRUE)
  ...
  #########################################
     ...
     for (p in intersect(op_echan$alpha,which(h$delta == 1))) { 
       ...
       for (k in 1:(h$K[p])) { 
         ...
         if (h$alpha[[p]][k] == 0) { h_c$lalpha[[p]][k] <- rnorm(1,mu_lalpha,s_lalpha); p_Z_move = c(1);
         }else{
           p_Z_move = 1 - exp(-h$alpha[[p]][k]) 
       ...
       #------------ proba of move  
         ...
         if (!test_rho_h_c) {test_accept = FALSE}
         if (test_rho_h_c) { 
           if ((sum(h_c$alpha[[p]] == 0) == h_c$K[p])) { proba_accept = -Inf 
           }else{
             p_Z_move_c = 1 - exp(-h_c$alpha[[p]][k]) 
