...
RJ_Kernel_moving_s_positive_intensity = function(data,
                                                 INPUT,
                                                 hyperParam_prior,
                                                 par_algo_MCMC) {
  #op.plot=TRUE
  ...
  log_lik = log_likelihood_hawkes_multidim_positive_intensity(data, idx, J, h, nu, op =
                                                                'vec')
  ...
  for (it in 2:N_MCMC) {
    ...
    for (m in op_echan$nu) {
      ...
      log_lik_c  = log_likelihood_hawkes_multidim_positive_intensity(data, idx, J, h, nu_c, op =
                                                                       'vec')
    ...
    if (length(op_echan$nu) == M) {
      rho_lnu <- 0.2
      eval_lambda <-
        lambda_cond_multidim_mat_positive_intensity(mat_obs_utiles, idx, h, nu)
      grad_log_post <-
        vapply(1:M, function(m) {
          nu[m] * sum(1 / eval_lambda[[m]])
        }, 1) - 1 / s_lnu ^ 2 * (lnu - mu_lnu) - nu * (Tmax - Tinf)
      lnu_c <-
        lnu + rho_lnu * grad_log_post + sqrt(2 * rho_lnu) * rnorm(M)
      
      nu_c = exp(lnu_c)
      log_lik_c <-
        log_likelihood_hawkes_multidim_positive_intensity(data, idx, J, h, nu_c, op =
                                                            "vec")
      log_prior_lnu_c <- dnorm(lnu_c, mu_lnu, s_lnu, log = TRUE)
      q_nu_c_nu <-
        sum(dnorm(lnu_c, lnu + rho_lnu * grad_log_post, 2 * rho_lnu, log = TRUE))
      
      eval_lambda_c <-
        lambda_cond_multidim_mat_positive_intensity(mat_obs_utiles, idx, h, nu_c)
      grad_log_post_c <-
        vapply(1:M, function(m) {
          sum(nu_c[m] / eval_lambda_c[[m]])
        }, 1) - 1 / s_lnu ^ 2 * (lnu_c - mu_lnu) - nu_c * (Tmax - Tinf)
    ...
    #     h_c$delta[p] <- 1
    #
    #     h_c$K[p] <- rpois(1, lambda_K) + 1
    #
    #     h_c$lalpha[[p]] <-
    #       rprior_lalpha(h_c$K[p], mu_lalpha, s_lalpha, p_Z)
    ...
    #     h_big = h_c
    #
    #     q_small_to_big = dprior_s(h_big$s[[p]], hyperParam_prior$a_s, log =
    #                                 TRUE) + dpois(h_big$K[p] - 1, lambda_K, log = TRUE) + dprior_lalpha(h_big$lalpha[[p]], mu_lalpha, s_lalpha, p_Z, log =
    #                                                                                                       TRUE)
    #
    #     proba_accept = sum(log_lik_c - log_lik) + log_prior_h_c$h - log_prior_h$h -
    #       q_small_to_big
    #   }
    #   if (move == 'decrease') {
    #     h_small = h_c
    ...
    for (p in op_echan$alpha) {
      ...
      for (k in 1:(h$K[p])) {
        ...
        if ((sum(h_c$alpha[[p]] == 0) == h_c$K[p])) {
          ...
          #
          log_prior_h_c = dprior_h_moving_s(h_c, hyperParam_prior)
          log_lik_c = log_likelihood_hawkes_multidim_positive_intensity(data, idx, J, h_c, nu, op =
                                                                          'vec')
    ...
    for (p in op_echan$K) {
      ...
      log_prior_h_c = dprior_h_moving_s(h_c, hyperParam_prior)
      log_lik_c[m] = log_likelihood_hawkes_multidim_positive_intensity_m(data, idx_c, J_c, h_c, nu, m)
      proba_accept = log_lik_c[m] - log_lik[m] + log_prior_h_c$h - log_prior_h$h -
        log_prob_proposal
