...
source('functions/likelihood_intervention.R')
source('functions/RJ_Kernel_changepoint.R')
...
Times = read.csv("data/germany-deaths-16april.csv")[2]
c(Times[[1]][1], tail(Times[[1]],n=1))
```

```{r}
smax=3.0
Tmax = 33 # tail(Times[[1]],n=1);
Tinf = 2; 
...
data = list() #data=list(mat_obs_utiles=mat_obs_utiles);
...
data$tau=18   # lockdown in germany
```

```{r}
# Plots of the (smoothed) count of deaths per day
n_days = round(Times)
deaths = list()
...
#------  PRIOR DISTRIBUTION
#-------------------------------
p_Z = c(0.5,0.5);   # probabilite pallier non nul/nul?
p_Z=p_Z/sum(p_Z)
pi_0 = 0.5;

mu_lalpha =2.5; # hyperparametres prior sur hauteur des h
s_lalpha = 1

mu_lnu = rep(1,M); # hyperparametres prior sur nu
s_lnu=rep(1,M)

mu_lbeta = rep(1,M); # hyperparametres prior sur beta
s_lbeta=rep(1,M)

mu_llag = rep(1.5,M); # hyperparametres prior sur lag L
s_llag=rep(1.5,M)

hyperParam_prior=list(M=M,mu_lalpha=mu_lalpha,s_lalpha=s_lalpha,mu_lnu=mu_lnu,s_lnu=s_lnu,
                      mu_lbeta=mu_lbeta,s_lbeta=s_lbeta,mu_llag=mu_llag,s_llag=s_llag,
                      p_Z=p_Z,a_s=2,pi_0=pi_0)   # a_s????
...
#------ tuning 
N_MCMC = 100
op_echan = list(alpha = c(1:M^2),K = c(1:M^2),nu = c(1:M),lambda_K = c(1),delta = c(),s = c(1:M^2), beta = c(1:M), L = c(1:M))
...
h_init$K =  rep(3,M^2) #  rep(1,M^2)
h_init$s =   list();
for (p in  1:(M^2)) {h_init$s[[p]] =   seq(0,h_init$smax[p],length = h_init$K[p] + 1)}

h_init$alpha = lapply(1:M^2,function(p){rep(0.02,h_init$K[p])})
h_init$lalpha = lapply(1:M^2,function(p){rep(log(0.02),h_init$K[p])}) 
nu_init  =  vapply(1:M,function(m){0.002 * length(data$Times_obs[[m]])/(data$Tmax[m] - data$Tinf[m])},1)

beta_init = rep(0.9,M)
L_init = rep(3,M)

theta_init =   list(h =   h_init,nu =   nu_init, lambda_K = 3.0, beta = beta_init, Lag = L_init)
...
Tinter = data$tau + L_init
Times_obs_1=lapply(1:M,function(m){u_m=Times[[m]]; u_m = u_m[u_m>Tinf]; u_m=u_m[u_m <= Tinter]})
Times_obs_2=lapply(1:M,function(m){u_m=Times[[m]]; u_m = u_m[u_m>min(Tmax,Tinter)]; u_m=u_m[u_m <= Tmax]})
  
Times_utiles_1=lapply(1:M,function(m){u_m=Times[[m]]; u_m=u_m[u_m > Tinf-max(smax)];  u_m=u_m[u_m <= Tinter]})
mat_obs_utiles_1 =calc_mat_absc_utiles(Times_utiles_1,Times_obs_1,smax)
  
Times_utiles_2=lapply(1:M,function(m){u_m=Times[[m]]; u_m=u_m[u_m > min(Tmax,Tinter)-max(smax)];  u_m=u_m[u_m <= Tmax]})
mat_obs_utiles_2 =calc_mat_absc_utiles(Times_utiles_2,Times_obs_2,smax)

idx_1 <- func_idx(mat_obs_utiles_1, h_init$s)
J_1  <- calc_J(Times_utiles_1, h_init$s, Tinf, Tinter)
idx_2 <- func_idx(mat_obs_utiles_2, h_init$s)
J_2  <- calc_J(Times_utiles_2, h_init$s, Tinter, Tmax)

log_lik = log_likelihood_hawkes_multidim_positive_intensity_2(data,  mat_obs_utiles_1,  mat_obs_utiles_2,
                                                              idx_1, idx_2, J_1, J_2, h_init, nu_init, beta_init, L_init, op ='vec')
...
resMCMC = RJ_Kernel_changepoint(data,INPUT,hyperParam_prior,par_algo_MCMC)
resMCMC$INPUT <- INPUT
